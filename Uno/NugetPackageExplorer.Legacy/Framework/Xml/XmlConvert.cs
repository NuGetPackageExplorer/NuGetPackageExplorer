        ...
        private static object PopulateTo(this XElement element, object instance)
        {
            ...
            foreach (var attribute in element.Attributes())
            {
                ...
                if (property != null)
                {
                    ...
                    {
                        Logger.Value.Error($"Unknown property type: {property.PropertyType} <- @{attribute.Name.LocalName}");
                    }
                }
                else
                {
                    Logger.Value.Error($"Unmapped property: @{attribute.Name.LocalName}");
                }
            }
            foreach (var child in element.Elements())
            {
                ...
                if (property != null)
                {
                    ...
                    {
                        Logger.Value.Error($"Unknown property type: {property.PropertyType} <- {child.Name.LocalName}");
                    }
                }
                ...
                {
                    Logger.Value.Error($"Unmapped property: {child.Name.LocalName}");
                }
            }
